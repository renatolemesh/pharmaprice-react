...
import { fetchTrendsData } from '../../services/Api'; 

const mockDataGross = await fetchTrendsData();
const mockData = mockDataGross.data.map(({ date, increases, decreases }) => ({ day: new Date(date).toLocaleString('pt-BR', { weekday: 'short' }), date, increases, decreases }));
...
export const TrendsChart = () => {
  return (
    <Card className="bg-gradient-to-br from-card to-card/50 border-0 shadow-card">
      <CardHeader>
        <CardTitle className="text-lg font-semibold">Tendências de Movimento de Preços (última semana)</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="h-80">
